...
本文定义 `MoonBox` 的数据库迁移兼容规则，用于约束 Schema 变更如何设计、生成、执行、验证、回滚和归档。
...
- 数据库适配说明：`compatibility/database/MoonBox`
...
工程初始化生成本文时，应优先使用用户输入和自动派生配置填充以下参数。缺失信息必须标记为 `见 docs/pending-decisions.md`，不得编造数据库版本、迁移工具或生产策略。
...
| `SQLite` | 主数据库、版本、驱动、ORM/DAO | SQLite / PostgreSQL / MySQL / 达梦 |
| `MoonBox` | 主关系型数据库 | SQLite |
| `MoonBox` | 信创或兼容数据库目标 | 无 / 达梦 / 海量 / PostgreSQL |
| `MoonBox` | 迁移工具 | Alembic / Prisma / Flyway / Liquibase / 手写 SQL |
| `Alembic` | 迁移策略 | versioned migration / repeatable migration / ORM migration |
| `src/backend/app/models 与 migrations` | Schema 事实源 | migration / ORM schema / DDL / introspection |
| `MoonBox` | migration 文件目录 | `src/backend/migrations` |
| `MoonBox` | seed 或初始化数据目录 | `src/backend/seeds` |
| `MoonBox` | 回滚策略 | down migration / restore backup / forward fix |
| `MoonBox` | 备份策略 | 自动备份 / 手工备份 / 不适用 |
| `docker-compose` | 部署方式 | docker-compose / Kubernetes / 离线包 |
...
| `MoonBox` | 停机容忍度 | 可停机 / 不可停机 / 见 docs/pending-decisions.md |


当前项目迁移事实源：

```text
src/backend/app/models 与 migrations
```
...
当项目使用框架约定目录时，以 `MoonBox` 官方约定为准，但必须在本文记录。
...
当 `MoonBox` 不为 `无` 或项目声明支持多数据库时，推荐目录：
...
|---|---|---|---|---|
| 主键 | `MoonBox` | `MoonBox` | `MoonBox` | 见 docs/pending-decisions.md |
| 时间字段 | `MoonBox` | `MoonBox` | `MoonBox` | 见 docs/pending-decisions.md |
| JSON 字段 | `MoonBox` | `MoonBox` | `MoonBox` | 见 docs/pending-decisions.md |
| 全文搜索 | `MoonBox` | `MoonBox` | `MoonBox` | 见 docs/pending-decisions.md |
| 分页 | `MoonBox` | `MoonBox` | `MoonBox` | 见 docs/pending-decisions.md |
...
|---|---|---|---|---|
| local | `MoonBox` | `MoonBox` | 开发数据/空库 | 初始化成功 |
| test | `MoonBox` | `MoonBox` | 测试库/fixtures | 自动化测试通过 |
| staging | `MoonBox` | `MoonBox` | 生产近似数据 | 演练记录 |
| production | `MoonBox` | `MoonBox` | 生产数据 | 审批、备份、验收 |
...
| 验证项 | 要求 | 状态 |
|---|---|---|
| 空库初始化 | 从零创建数据库并完成所有迁移 | 见 docs/pending-decisions.md |
| 旧库升级 | 从上一稳定版本升级到当前版本 | 见 docs/pending-decisions.md |
| 幂等性 | 重复执行不会破坏数据或结构 | 见 docs/pending-decisions.md |
| 回滚/补偿 | 回滚、恢复或 forward fix 可执行 | 见 docs/pending-decisions.md |
| 数据完整性 | 行数、约束、枚举、关联关系正确 | 见 docs/pending-decisions.md |
| 应用兼容 | API、Repository、前端类型可正常运行 | 见 docs/pending-decisions.md |
...
- 识别当前 `MoonBox`、`src/backend/app/models 与 migrations` 和 `MoonBox`。
- 根据变更影响同步更新 Model、Repository、API Schema、测试和文档。
- 对无法确认的数据库版本、方言能力、生产策略标记 `见 docs/pending-decisions.md`。
