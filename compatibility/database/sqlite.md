...
本文定义 `MoonBox` 中 SQLite 的使用边界、适配规则、兼容差异和测试要求。
...
工程初始化生成本文时，应优先使用用户输入和自动派生配置填充以下参数。缺失信息必须标记为 `见 docs/pending-decisions.md`，不得编造 SQLite 版本、驱动或生产策略。
...
| `SQLite` | 数据库技术栈 | SQLite + SQLAlchemy |
| `MoonBox` | SQLite 版本 | 见 docs/pending-decisions.md |
| `MoonBox` | SQLite 驱动 | sqlite3 / aiosqlite / better-sqlite3 |
| `MoonBox` | ORM 或 DAO 技术栈 | SQLAlchemy / Prisma / Drizzle / 手写 SQL |
| `MoonBox` | SQLite 使用范围 | local / test / desktop / edge / production |
| `MoonBox` | 数据库文件路径 | `data/MoonBox.db` |
| `MoonBox` | 测试数据库路径 | `data/test.db` / in-memory |
| `MoonBox` | 迁移工具 | Alembic / Prisma / 手写 SQL |
| `MoonBox` | 迁移目录 | `src/backend/migrations` |
| `MoonBox` | 备份策略 | 文件备份 / 快照 / 不适用 |
| `MoonBox` | 并发模型 | 单进程 / 多进程 / 多实例 |
| `docker-compose` | 部署方式 | docker-compose / 桌面端 / 离线包 |
| `MoonBox` | 兼容数据库目标 | 无 / 达梦 / 海量 / PostgreSQL |
...
| 项 | 当前配置 | 说明 |
|---|---|---|
| SQLite 版本 | `MoonBox` | 需记录开发、测试、部署环境实际版本 |
| 驱动 | `MoonBox` | 与后端语言和 ORM 保持一致 |
| ORM / DAO | `MoonBox` | 与 `rules/database.md` 一致 |
| 数据库文件 | `MoonBox` | 运行时文件不得提交 Git |
| 测试数据库 | `MoonBox` | 推荐独立文件或内存库 |
| 迁移工具 | `MoonBox` | 与迁移规范一致 |
...
|---|---|---|---|
| 主键 ID | `MoonBox` | `MoonBox` | 自增整数、UUID 字符串或项目统一 ID |
...
| 验证项 | 要求 | 状态 |
|---|---|---|
| 空库初始化 | 从零创建 SQLite 数据库并完成迁移 | 见 docs/pending-decisions.md |
| 旧库升级 | 从上一版本 SQLite 数据库升级 | 见 docs/pending-decisions.md |
| 外键约束 | 外键启用且约束行为符合预期 | 见 docs/pending-decisions.md |
| 索引与查询 | 分页、筛选、排序、模糊搜索通过 | 见 docs/pending-decisions.md |
| 数据类型 | 布尔、时间、小数、JSON 映射正确 | 见 docs/pending-decisions.md |
| 事务回滚 | 失败时事务可回滚 | 见 docs/pending-decisions.md |
| 并发写入 | 符合项目并发模型 | 条件启用 |
...
当 `MoonBox` 包含 production、私有化轻量部署或桌面端正式数据时，必须完成以下准入：

| 准入项 | 要求 | 状态 |
|---|---|---|
| 数据规模 | 预估数据量和增长速度可接受 | 见 docs/pending-decisions.md |
| 并发模型 | 写入并发可控 | 见 docs/pending-decisions.md |
| 文件系统 | 持久卷、权限和锁行为可靠 | 见 docs/pending-decisions.md |
| 备份恢复 | 已演练备份和恢复 | 见 docs/pending-decisions.md |
| 迁移演练 | 已验证版本升级路径 | 见 docs/pending-decisions.md |
| 监控告警 | 文件大小、磁盘、错误日志可观察 | 见 docs/pending-decisions.md |
| 替代方案 | 高并发或规模增长时有迁移路径 | 见 docs/pending-decisions.md |
...
- 识别当前 `MoonBox`、`MoonBox`、`MoonBox` 和 `MoonBox`。
- 同步更新迁移、Model、Repository、API Schema、测试和文档。
- 对无法确认的 SQLite 版本、扩展能力、生产备份策略标记 `见 docs/pending-decisions.md`。
