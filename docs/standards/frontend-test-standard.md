...
owner: MoonBox
note: 适用于 MoonBox 项目；无前端项目可保留为未来启用规范并标记不适用
---
...
| `MoonBox` | 前端负责人 | 见 docs/pending-decisions.md |
| `MoonBox` | 是否启用前端 | true / false |
| `React + TypeScript + TailWind + Shadcn/UI + Axios + Orval + pnpm` | 前端技术栈 | React / Vue / Svelte / Angular / WeChat Miniapp / Flutter Web / none |
| `MoonBox` | 前端测试工具 | Vitest / Jest / Testing Library / Cypress CT / Playwright CT / none |
| `MoonBox` | E2E 测试工具 | Playwright / Cypress / WebdriverIO / none |
| `MoonBox` | 前端源码目录 | `src/web` / 见 docs/pending-decisions.md |
| `MoonBox` | 测试文件匹配 | 见 docs/pending-decisions.md |
| `MoonBox` | 前端测试命令 | 见 docs/pending-decisions.md |
| `MoonBox` | 类型检查/Lint/格式命令 | 见 docs/pending-decisions.md |
| `MoonBox` | 前端覆盖率命令 | 见 docs/pending-decisions.md |
| `MoonBox` | 前端覆盖率目标 | 见 docs/pending-decisions.md |
| `MoonBox` | 是否启用 Design System | true / false |
| `MoonBox` | 组件库路径 | 见 docs/pending-decisions.md |
| `MoonBox` | API Mock 策略 | MSW / mock adapter / fake client / none |
| `MoonBox` | 视觉测试策略 | screenshot / storybook / manual / none |
| `MoonBox` | 可访问性测试策略 | automated / checklist / none |
...
当 `MoonBox=false` 且项目无前端界面时，可保留本文档为未来启用规范，并删除强制测试命令、目录、组件测试和 E2E 要求。
...
```text
React + TypeScript + TailWind + Shadcn/UI + Axios + Orval + pnpm
```
...
- 当前项目未配置前端测试框架时，应标记为 `见 docs/pending-decisions.md`，不得编造命令。
...
- 命令必须来自实际 `package.json`、脚本或 CI 配置；未知时标记 `见 docs/pending-decisions.md`。
...
1. 根据用户输入替换 `MoonBox`、`MoonBox`、`MoonBox`、`React + TypeScript + TailWind + Shadcn/UI + Axios + Orval + pnpm`、`MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`。
4. 用真实测试框架、目录、命令、组件路径、Mock 策略和覆盖率目标替换占位；未知信息标记为 `见 docs/pending-decisions.md`。
