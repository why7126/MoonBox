...
owner: MoonBox
note: 适用于 MoonBox 项目；错误码必须与 API、认证、安全、前端处理和测试保持一致
---
...
| `MoonBox` | 错误码风格 | numeric / string / mixed |
| `MoonBox` | 成功码 | `0` |
| `MoonBox` | 统一响应结构 | `{ code, message, data }` |
| `MoonBox` | 错误码分段规则 | 见 docs/pending-decisions.md |
| `MoonBox` | 认证授权错误码段 | 见 docs/pending-decisions.md |
| `MoonBox` | 业务错误码段 | 见 docs/pending-decisions.md |
| `MoonBox` | 参数错误码段 | 见 docs/pending-decisions.md |
| `MoonBox` | 外部依赖错误码段 | 见 docs/pending-decisions.md |
| `MoonBox` | 错误码常量实现路径 | 见 docs/pending-decisions.md |
| `MoonBox` | 异常与响应处理路径 | 见 docs/pending-decisions.md |
| `MoonBox` | 前端错误处理路径 | 见 docs/pending-decisions.md |
| `MoonBox` | 错误码校验命令 | 见 docs/pending-decisions.md |


满足以下任一条件时，应完整启用本文档：

- 项目暴露 API、SDK、Webhook、RPC、GraphQL 或对外集成能力。
- 项目有前端、移动端、微信小程序或第三方调用方需要基于错误码处理状态。
...
默认建议使用数字分段。初始化时可根据 `MoonBox` 和 `MoonBox` 调整。

| 区间/前缀 | 含义 | 是否通用 | 负责人 | 备注 |
|---|---|---|---|---|
| `MoonBox` | 成功 | 通用 | `MoonBox` | 默认成功 |
| `1xxxx` | 系统错误 | 通用 | `MoonBox` | 未预期异常、服务内部错误 |
| `MoonBox` | 认证授权 | 条件启用 | `MoonBox` | 登录、Token、权限、账号状态 |
| `MoonBox` | 业务错误 | 个性化 | `MoonBox` | 按业务域生成 |
| `MoonBox` | 参数错误 | 通用 | `MoonBox` | 参数格式、分页、枚举、字段校验 |
| `MoonBox` | 外部依赖 | 条件启用 | `MoonBox` | 存储、第三方服务、消息队列等 |
...
|---|---|---|---|---|---|---|---|---|
| `MoonBox` | 200 | success | success | 操作成功 | 成功响应 | 否 | 正常处理 | active |
| `MoonBox` | `MoonBox` | `MoonBox` | `MoonBox` | `MoonBox` | `MoonBox` | `MoonBox` | `MoonBox` | `MoonBox` |
...
1. 根据用户输入替换 `MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`。
4. 根据产品能力、需求和 API 设计生成业务错误码；未知时标记为 `见 docs/pending-decisions.md`。
5. 用项目真实实现路径替换 `MoonBox`、`MoonBox`、`MoonBox`；未知时标记为 `见 docs/pending-decisions.md`。
