...
| `MoonBox` | 媒体与上传负责人 | 见 docs/pending-decisions.md |
| `MoonBox` | 是否启用上传 | true / false |
| `MoonBox` | 上传业务场景 | 头像、附件、导入文件、视频等 |
| `MoonBox` | 上传方式 | multipart / presigned-url / chunked / mixed |
| `MoonBox` | API 基础前缀 | `/api/v1` |
| `MoonBox` | 上传接口清单 | 见 docs/pending-decisions.md |
...
| `MoonBox` | 上传安全策略 | MIME、文件头、扫描、隔离等 |
| `MoonBox` | 上传响应结构 | 见 docs/pending-decisions.md |
| `MoonBox` | 上传错误码 | 见 docs/pending-decisions.md |
| `MoonBox` | 上传测试命令 | 见 docs/pending-decisions.md |
...
当 `MoonBox=false` 且项目无任何上传、导入、附件、媒体或对象存储写入能力时，可保留本文档为未来启用规范，并删除强制接口、环境变量、存储和测试要求。
...
```http
POST MoonBox/uploads/{resource_type}
MoonBox
...
```http
POST MoonBox/uploads/{resource_type}/presign
MoonBox
...
|---|---|---|---:|---|---|
| 图片 | JPEG、PNG、WebP | `MoonBox` | `MoonBox` | 分辨率、EXIF、压缩 | 见 docs/pending-decisions.md |
| 音频 | MP3、WAV、M4A、WebM | `MoonBox` | `MoonBox` | 时长、采样率 | 见 docs/pending-decisions.md |
| 视频 | MP4、MOV、WebM | `MoonBox` | `MoonBox` | 时长、分辨率、封面 | 见 docs/pending-decisions.md |
| 文档 | PDF、DOCX、XLSX | `MoonBox` | `MoonBox` | 预览、病毒扫描 | 见 docs/pending-decisions.md |
| 导入文件 | CSV、XLSX、ZIP | `MoonBox` | `MoonBox` | 行数、压缩包安全 | 见 docs/pending-decisions.md |
...
|---|---|---|
| `MoonBox` | 文件或媒体 ID | 是 |
| `MoonBox` | 所属用户、租户或组织 | 是 |
| `MoonBox` | 业务对象引用 | 条件启用 |
| `MoonBox` | 对象 Key | 是 |
| `MoonBox` | 原始文件名 | 条件启用，需脱敏/清洗 |
| `MoonBox` | MIME 类型 | 是 |
| `MoonBox` | 文件大小 | 是 |
| `MoonBox` | 文件哈希 | 条件启用 |
| `MoonBox` | 上传/处理状态 | 条件启用 |
| `MoonBox` | 创建时间 | 是 |
...
1. 根据用户输入替换 `MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`、`MinIO 兼容对象存储，面向文档与图片资产`、`MoonBox`、`MoonBox`。
4. 用真实上传场景、接口、文件类型、大小限制、错误码和元数据字段替换占位；未知信息标记为 `见 docs/pending-decisions.md`。
