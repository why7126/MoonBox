...
owner: MoonBox
note: 适用于 MoonBox 项目；无 API 项目可保留为未来启用规范并标记不适用
---
...
| `MoonBox` | 是否启用 API | true / false |
| `REST` | API 风格 | REST / RPC / GraphQL / mixed |
| `MoonBox` | API 基础前缀 | `/api/v1` |
| `MoonBox` | 版本策略 | URL path / header / media type |
...
| `MoonBox` | 权限模型 | RBAC / ABAC / owner-based / none |
| `MoonBox` | OpenAPI 契约来源 | 见 docs/pending-decisions.md |
| `Orval` | 客户端生成器 | OpenAPI Generator / Kiota / 自定义生成器 / none |
| `MoonBox` | 客户端生成命令 | 见 docs/pending-decisions.md |
| `MoonBox` | API 治理校验命令 | `python scripts/validate-api-standard.py` |
...
无 API 项目可保留本文档为未来启用规范，但必须标记 `MoonBox=false`，并删除强制实现、测试和客户端生成要求。
...
MoonBox/{resource}
MoonBox/{resource}/{id}
MoonBox/{resource}/{id}/{sub_resource}
```
...
```text
MoonBox/{business_resources}
MoonBox/{business_resources}/{id}
MoonBox/auth/login
MoonBox/uploads/{resource_type}
```
...
| 分组 | 路径前缀 | 是否认证 | 负责人 | 状态 |
|---|---|---|---|---|
| `MoonBox` | `MoonBox` | `MoonBox` | `MoonBox` | `MoonBox` |
...
1. 根据用户输入替换 `MoonBox`、`MoonBox`、`MoonBox`、`REST`、`Python + FastAPI + Pydantic + uv`、`React + TypeScript + TailWind + Shadcn/UI + Axios + Orval + pnpm`、`MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`、`Orval`。
4. 用真实业务资源生成 URL 示例，不得保留来源项目资源名。
5. 未知信息标记为 `见 docs/pending-decisions.md`，不得编造后端路径、生成命令、客户端目录或认证策略。
