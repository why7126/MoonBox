...
owner: MoonBox
note: 适用于 MoonBox 项目；本文档聚焦单元测试，测试治理总纲见 testing-governance
---
...
| `MoonBox` | 单元测试框架 | pytest / unittest / vitest / jest / go test / junit |
| `MoonBox` | 单元测试命令 | 见 docs/pending-decisions.md |
| `MoonBox` | 单元测试目录 | 见 docs/pending-decisions.md |
| `MoonBox` | 测试文件命名模式 | 见 docs/pending-decisions.md |
| `MoonBox` | 代码分层 | API / Service / Repository / Domain / UI / Shared |
| `MoonBox` | Mock 策略 | 见 docs/pending-decisions.md |
| `MoonBox` | Fixture/Factory 策略 | 见 docs/pending-decisions.md |
| `MoonBox` | 单元测试覆盖率目标 | 见 docs/pending-decisions.md |
| `MoonBox` | CI 单元测试门禁 | required / warning / disabled |
...
当项目暂未具备自动化单元测试条件时，应保留本文档并标记 `MoonBox=见 docs/pending-decisions.md`，同时记录未来启用条件。
...
- 命令必须来自实际脚本、测试框架或 CI 配置；未知时标记 `见 docs/pending-decisions.md`。
...
1. 根据用户输入替换 `MoonBox`、`MoonBox`、`MoonBox`、`Python + FastAPI + Pydantic + uv`、`React + TypeScript + TailWind + Shadcn/UI + Axios + Orval + pnpm`、`MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`。
2. 根据 SOURCE_LAYERING、BACKEND_STACK、FRONTEND_STACK、HAS_SDK、HAS_ALGORITHM、HAS_SCRIPTS 生成必须覆盖对象。
3. 根据实际语言和测试框架改写目录、命名、示例和命令。
5. 未确认的信息标记为 `见 docs/pending-decisions.md`，不得编造测试框架、命令、目录、覆盖率目标或 CI 门禁。
