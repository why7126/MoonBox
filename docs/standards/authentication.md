...
| 后台管理员 | Web 前台、管理后台 | 登录前台、进入需求中心、从前台用户菜单进入后台、用户列表、创建、编辑、冻结、解冻、逻辑删除、重置密码、头像上传；待激活时可首次登录激活 | 操作系统内置唯一超级管理员 | 平台账号元数据 | 通过 `Authorization: Bearer <access_token>` 和服务端会话记录认证；后台接口必须按当前角色二次授权 |
| 前台用户 | Web 前台 | 登录前台、进入需求中心；待激活时可首次登录激活 | 访问管理后台用户管理接口、看到或使用“进入后台”入口 | 自身业务范围 | 使用统一登录态，但不具备后台授权 |
| 系统内置超级管理员 | Web 前台、管理后台 | 登录前台、进入后台、平台恢复与最高权限身份展示 | 在用户管理中被编辑、重置密码、冻结、解冻或删除 | 平台级 | 由部署初始化创建，当前角色仍必须是后台管理员 |
...
| 项 | 规则 |
|---|---|
| 登录态存储 | 统一 Web 会话存储在 `moonbox.session`，包含服务端可校验 token 与用户摘要；前台和后台共同读取同一份 session，不得进入 URL、日志或埋点 |
| 登录页路径 | `/login` 是统一登录入口；管理后台 `/admin` 未登录或无后台权限时引导回统一登录页，旧 `#admin-users` 暂时兼容 |
| 登录后跳转 | 登录成功后默认进入前台 `/requirements` |
| 退出登录处理 | 调用 `/api/v1/auth/logout` 撤销当前服务端会话，并清理统一本地登录态 |
| 路由守卫策略 | `/requirements` 需要统一登录态；`/admin` 需要统一登录态且用户当前角色为“后台管理员”；后端所有 `/api/v1/admin/**` 受保护接口仍以 Bearer token、服务端会话和后台角色授权为准 |
