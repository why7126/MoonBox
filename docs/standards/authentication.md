...
| `MoonBox` | 安全与认证负责人 | 见 docs/pending-decisions.md |
| `MoonBox` | 是否启用认证 | true / false |
| `MoonBox` | 认证策略 | Token / Session / OAuth2 / OIDC / SSO / API Key / none |
| `MoonBox` | 认证提供方 | 自研账号 / 企业 SSO / 第三方身份源 / none |
| `MoonBox` | 认证 Header 或 Cookie | `Authorization: Bearer <token>` / Cookie |
| `MoonBox` | Token 类型 | access token / refresh token / session id |
| `MoonBox` | Token 过期策略 | 见 docs/pending-decisions.md |
| `MoonBox` | 刷新策略 | 见 docs/pending-decisions.md |
| `MoonBox` | 前端存储策略 | HttpOnly Cookie / memory / controlled storage |
| `MoonBox` | 密码策略 | 见 docs/pending-decisions.md |
| `MoonBox` | 多因素认证策略 | disabled / optional / required |
| `MoonBox` | 权限模型 | RBAC / ABAC / owner-based / tenant-based / none |
| `MoonBox` | 角色矩阵 | 见 docs/pending-decisions.md |
| `MoonBox` | 公开端点清单 | 健康检查、登录、回调等 |
| `MoonBox` | 认证授权错误码段 | `2xxxx` / 见 docs/pending-decisions.md |
| `MoonBox` | 认证测试命令 | 见 docs/pending-decisions.md |
...
当 `MoonBox=false` 且项目无任何登录态、用户身份、服务间凭证或访问控制需求时，可保留本文档为未来启用规范，并删除强制实现和测试要求。
...
- `MoonBox=none` 时，只允许公开、离线或本地工具类场景；不得用于有敏感数据或管理操作的系统。
...
1. 根据用户输入替换 `MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`、`MoonBox`。
4. 用真实端点、角色、权限和错误码替换示例占位；未知信息标记为 `见 docs/pending-decisions.md`。
