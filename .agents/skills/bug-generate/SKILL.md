...
**Input**：完整 `BUG-xxxx-slug`（须 `capture.md`）
...
`/bug-complete <BUG-full-id>`，其中 `<BUG-full-id>` MUST 使用完整 `BUG-xxxx-slug`。

---

## 当前态看板索引（MUST）

成功生成 `bug.md` 后，MUST 在 `issues/bugs/CHANGELOG.md` 更新对应 BUG 当前态行。普通文案润色、格式调整或错别字修复 MAY 不更新。
