...
`/spec-opt` 完成本项目规范、技能、脚本、目录边界或校验规则迭代后，MUST 在 `docs/spec-logs/` 写入治理迭代日志。
...
治理迭代日志 MUST 包含：迭代目标、变更摘要、影响范围、更新文件、验证结果、API/DB/Web/客户端/管理端/Orval/Docker Compose 影响和后续建议。
...
- 如果存在明确可推进的下一步，MUST 给出可复制执行的命令；REQ 来源链路使用完整 `REQ-xxxx-slug`，BUG 来源链路使用完整 `BUG-xxxx-slug`，非 REQ/BUG Change 才使用 `<change-id>`。
