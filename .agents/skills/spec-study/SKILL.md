...
学习对象可以是本地项目路径或 GitHub 项目 URL。本地路径只用于本次会话内定位和只读读取；任何持久化文档、active Change trace、治理日志或最终学习报告 MUST 使用项目名或脱敏占位符记录学习对象，不得写入本机绝对路径、系统用户名或用户主目录。
...
- `src/` 下任何业务运行时代码
- 后端 API、数据库 schema、Web、客户端、管理端业务实现
...
8. 输出学习报告。报告 MUST 统一写入 `docs/spec-logs/`，文件名 MUST 使用 `YYYYMMDDhhmmss-study-xxx.md` 格式，例如 `20260807103000-study-design-system.md`；同一次学习应用流程只生成一份正式 `study` 报告，不得同时生成内容重复的 `YYYYMMDDhhmmss-governance-xxx.md` 治理日志；若同一流程已有报告，MUST 更新该报告。不得写入 active Change 的 `implementation/` 或 `docs/knowledge-base/` 作为正式学习报告位置。
...
- `xxx` MUST 使用小写 kebab-case，表达学习对象或主题，例如 `pm-harness-study`、`design-system-study`、`api-governance-study`。
- Markdown Frontmatter MUST 包含 `created_at` 与 `updated_at`，时间格式遵守 `rules/document-governance.md`。
- 学习报告 MUST NOT 包含用户隐私数据、真实客户数据、密钥、访问令牌、未脱敏日志、订单原文、聊天原文、工单原文、截图中的个人信息、学习对象源码、本机绝对路径、系统用户名或用户主目录；涉及隐私风险时只能使用脱敏占位符或聚合描述。
- 学习对象为本地路径时，报告、trace 和最终回复 SHOULD 使用 `ProjectName（本地只读项目）` 或 `<local-project>/ProjectName`；MUST NOT 写入 `/Users/<name>/...`、`/home/<name>/...`、用户主目录、系统用户名或可反推出个人环境的目录结构。
