...
def compose_tag_defaults() -> dict[str, list[str]]:
    ...
    pattern = re.compile(r"\$\{(?:MOONBOX_IMAGE_TAG|IMAGE_BUILD_TAG|[A-Z0-9_]+_IMAGE_TAG):-([^}]+)\}")
    for relative in ("docker-compose.yml", "docker-compose.prod.yml", "docker-compose.prod.external.yml"):
        path = ROOT / relative
        if path.exists():
                ...
                warnings.append(
                    {
                        ...
                        "message": f"{compose_file} fallback image tag is {tag}; release deploy env must set MOONBOX_IMAGE_TAG or IMAGE_BUILD_TAG to {version}.",
                    }
                )
...
def build_images(version: str, release_dir: Path | None = None, env_file: Path = DEFAULT_ENV_FILE) -> dict[str, Any]:
    ...
    backend_name = env_values.get("IMAGE_BUILD_BACKEND_IMAGE", "moonbox-backend")
    web_name = env_values.get("IMAGE_BUILD_WEB_IMAGE", "moonbox-web")
    ...
    tar_name = env_values.get("IMAGE_BUILD_TAR_NAME") or f"moonbox-{tag}-{platform.replace('/', '-')}.tar.gz"
