...
系统 MUST 提供管理后台登录入口，支持系统内置超级管理员使用用户名和密码登录后台。
...
- **WHEN** 用户提交错误用户名或密码
- **THEN** 系统返回受控认证失败错误
- **AND** 响应不得泄露账号是否存在、密码哈希算法、内部用户状态或系统路径
...
- **THEN** 前端必须提示临时密码仅可用于状态为正常且具备后台管理员角色的账号登录后台
- **AND** 前端不得暗示前台用户、待激活、已冻结或已删除账号可凭临时密码登录后台
